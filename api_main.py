...
def pipeline(req):
    try:
        ...
        run_stage(["python","src/module1_ingest_parse.py",req.log_file,"--dataset",req.dataset,"--max-lines","100000"],"Module 1: Ingestion")
        cmd2=["python","src/module2_session_anomaly.py",csv_f,"--dataset",req.dataset,"--contamination",str(req.contamination)]
        if Path(req.label_path).exists(): cmd2+=["--label-path",req.label_path]
        run_stage(cmd2,"Module 2: Anomaly Detection")
        run_stage(["python","src/module3_embed_index.py",json_f,"--dataset",req.dataset,"--model","all-MiniLM-L6-v2"],"Module 3: FAISS")
        if not req.offline:
            run_stage(["python","src/module4_rag_analysis.py",json_f,"--dataset",req.dataset,"--max-sessions",str(req.max_sessions)],"Module 4: LLM")
        state["log"].append("🎉 Pipeline complete!"); state["done"]=True
    except Exception as e:
        state["error"]=str(e); state["log"].append(f"❌ {e}")
    finally:
        state["running"]=False
...
def run(req:RunRequest,bg:BackgroundTasks):
    ...
    bg.add_task(pipeline,req); return {"status":"started"}

@app.get("/api/status")
def status(): return {**state,"log_tail":state["log"][-30:]}
